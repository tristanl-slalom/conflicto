...
from app.db.models import Session, SessionStatus, Activity, Participant
...
class SessionService:
    ...
    @staticmethod
    async def get_session(db: AsyncSession, session_id: int, include_relations: bool = False) -> Optional[Session]:
        """Get a session by ID."""
    ...
    @staticmethod
    async def get_session_by_code(db: AsyncSession, code: str, code_type: str = "qr") -> Optional[Session]:
    ...
    @staticmethod
    async def list_sessions(db: AsyncSession, offset: int = 0, limit: int = 100) -> List[Session]:
        """List sessions with pagination."""
        query = select(Session).offset(offset).limit(limit).order_by(Session.created_at.desc())
        result = await db.execute(query)
    ...
    async def update_session(db: AsyncSession, session_id: int, session_data: SessionUpdate) -> Optional[Session]:
        ...
        if session_data.status is not None:
            ...
            if old_status == SessionStatus.DRAFT and session.status == SessionStatus.ACTIVE:
                session.started_at = func.now()
            elif session.status == SessionStatus.COMPLETED and session.completed_at is None:
    ...
    @staticmethod
    async def _code_exists(db: AsyncSession, qr_code: str = None, admin_code: str = None) -> bool:
        ...
        if admin_code:
            result = await db.execute(select(Session).where(Session.admin_code == admin_code))
            return result.scalar_one_or_none() is not None
