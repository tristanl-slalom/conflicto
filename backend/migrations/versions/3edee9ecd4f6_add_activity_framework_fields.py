"""add_activity_framework_fields

Revision ID: 3edee9ecd4f6
Revises: 21f24228dbb3
Create Date: 2025-10-08 10:55:25.024303

"""
from alembic import op
import sqlalchemy as sa
from sqlalchemy.dialects import postgresql
from app.db.models import JSONBType

# revision identifiers, used by Alembic.
revision = '3edee9ecd4f6'
...
def upgrade() -> None:
    ...
    op.add_column('activities', sa.Column('title', sa.String(length=500), nullable=False, server_default='Untitled Activity'))
    op.add_column('activities', sa.Column('description', sa.Text(), nullable=True))
    op.add_column('activities', sa.Column('configuration', JSONBType(), nullable=False, server_default='{}'))
    op.add_column('activities', sa.Column('activity_metadata', JSONBType(), nullable=False, server_default='{}'))
    op.add_column('activities', sa.Column('state', sa.String(length=20), nullable=False, server_default='draft'))
    op.add_column('activities', sa.Column('expires_at', sa.DateTime(timezone=True), nullable=True))
    ...
    op.alter_column('participants', 'is_active',
               ...
               nullable=False)
    op.alter_column('participants', 'connection_data',
...
def downgrade() -> None:
    ...
    op.alter_column('activities', 'status',
               ...
               existing_nullable=False,
               existing_server_default=sa.text("'draft'::new_activity_status"))
    op.drop_column('activities', 'expires_at')
    op.drop_column('activities', 'state')
    op.drop_column('activities', 'activity_metadata')
    op.drop_column('activities', 'configuration')
