...
def upgrade() -> None:
    ...
    op.add_column('participants', sa.Column('nickname', sa.String(length=50), nullable=True))
    op.add_column('participants', sa.Column('last_seen', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True))
    op.add_column('participants', sa.Column('connection_data', sa.JSON(), nullable=True, default={}))
    
    # Update existing participants to have a default nickname based on display_name
    op.execute("UPDATE participants SET nickname = COALESCE(display_name, 'User' || id::text) WHERE nickname IS NULL")
