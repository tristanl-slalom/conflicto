...
class TestUserResponseService:
    ...
    async def test_create_response(
        self, db_session, sample_session_activity_participant, sample_user_response_data
    ):
    ...
    async def test_get_activity_responses_with_pagination(
        self, db_session, sample_session_activity_participant, sample_user_response_data
    ):
    ...
    async def test_get_participant_response_not_found(
        self, db_session, sample_session_activity_participant
    ):
    ...
    async def test_update_response(
        ...
        updated_response = await UserResponseService.update_response(
            db=db_session, response_id=response.id, response_data=update_data
        )
    ...
    async def test_delete_response(
        ...
        deleted = await UserResponseService.delete_response(
            db=db_session, response_id=response.id
        )
    ...
    async def test_get_response_summary(
        self, db_session, sample_session_activity_participant, sample_user_response_data
    ):
    ...
    async def test_get_response_summary_empty(
        self, db_session, sample_session_activity_participant
    ):
    ...
    async def test_get_responses_by_participant(
        self, db_session, sample_session_activity_participant, sample_user_response_data
    ):
        ...
        activity_data = ActivityCreate(
            type="survey", config={"question": "Another question?"}, order_index=2
        )
    ...
    async def test_get_responses_by_participant_with_pagination(
        self, db_session, sample_session_activity_participant, sample_user_response_data
    ):
        ...
        for i in range(3):
            activity_data = ActivityCreate(
                type=f"poll_{i}", config={"question": f"Question {i}?"}, order_index=i
            )
            activity = await ActivityService.create_activity(
                db_session, data["session_id"], activity_data
            )
    ...
    async def test_get_responses_since(
        self, db_session, sample_session_activity_participant, sample_user_response_data
    ):
    ...
    async def test_get_responses_since_empty(
        self, db_session, sample_session_activity_participant
    ):
    ...
    async def test_get_responses_since_with_limit(
        self, db_session, sample_session_activity_participant, sample_user_response_data
    ):
