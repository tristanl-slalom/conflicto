...
class TestActivityAPI:
    ...
    async def test_create_activity(
        self, async_client: AsyncClient, sample_session, sample_activity_data
    ):
        ...
        response = await async_client.post(
            f"/api/v1/sessions/{sample_session['id']}/activities", json=activity_data
        )
    ...
    async def test_create_activity_invalid_session(
        self, async_client: AsyncClient, sample_activity_data
    ):
        """Test creating activity for non-existent session."""
        response = await async_client.post(
            "/api/v1/sessions/99999/activities", json=sample_activity_data
        )
        assert response.status_code == 400

    async def test_get_session_activities(
        self, async_client: AsyncClient, sample_session
    ):
        ...
        await async_client.post(
            f"/api/v1/sessions/{session_id}/activities", json=activity1_data
        )
        await async_client.post(
            f"/api/v1/sessions/{session_id}/activities", json=activity2_data
        )
    ...
    async def test_update_activity(
        ...
        # Create activity
        create_response = await async_client.post(
            f"/api/v1/sessions/{session_id}/activities", json=sample_activity_data
        )
    ...
    async def test_activity_ordering(self, async_client: AsyncClient, sample_session):
        ...
        for i in range(3):
            ...
            await async_client.post(
                f"/api/v1/sessions/{session_id}/activities", json=activity_data
            )
    ...
    async def test_activity_status_filtering(
        self, async_client: AsyncClient, sample_session
    ):
        ...
        response = await async_client.get(
            f"/api/v1/sessions/{session_id}/activities?status=active"
        )
        activities = response.json()["activities"]
...
class TestActivityService:
    ...
    async def test_create_activity_service(self, db_session: AsyncSession):
        ...
        session_data = SessionCreate(
            title="Test Session", description="Test Description", max_participants=100
        )
        session = await SessionService.create_session(db_session, session_data)
        ...
        activity = await ActivityService.create_activity(
            db=db_session, session_id=session.id, activity_data=activity_data
        )
    ...
    async def test_get_activity_service(self, db_session: AsyncSession):
        ...
        retrieved_activity = await ActivityService.get_activity(
            db=db_session, activity_id=created_activity.id
        )
    ...
    async def test_update_activity_service(self, db_session: AsyncSession):
        ...
        updated_activity = await ActivityService.update_activity(
            db=db_session, activity_id=activity.id, activity_data=update_data
        )
    ...
    async def test_delete_activity_service(self, db_session: AsyncSession):
        ...
        success = await ActivityService.delete_activity(
            db=db_session, activity_id=activity.id
        )
