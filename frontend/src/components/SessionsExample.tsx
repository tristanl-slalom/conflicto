...
export function SessionsExample() {
  ...
  const { data: sessionsData, isLoading, error } = useListSessionsApiV1SessionsGet()
  ...
  const handleCreateSession = async () => {
    ...
    try {
      const result = await createSessionMutation.mutateAsync({ data: newSession })
  ...
  return (
    <div className="p-4">
      ...
      >
        {createSessionMutation.isPending ? 'Creating...' : 'Create Example Session'}
      </button>
        ...
        {sessionsData?.data.sessions.map((session) => (
          <div key={session.id} className="p-3 border rounded">
            ...
            <div className="text-sm text-gray-500">
              Status: {session.status} | Max Participants: {session.max_participants}
            </div>
          </div>
        ))}
      </div>
